...
struct HomeView: View {
    ...
    @State private var selectedSlide: Slide?
    
    var body: some View {
        VStack(spacing: 0) {
            header
            VTabView(selection: $selection) {
                if let slides = viewModel.slides {
                    ForEach(Array(slides.enumerated()), id: \.element) { index, slide in
                        ...
                        .task {
                            ...
                            if PagingUtil.isLastPage(index: index, pagingInterval: HomeViewModel.pagingInterval, count: slides.count) {
                                viewModel.getSlides()
                            }
                        }
                    }
        ...
        .sheet(item: $selectedSlide) { post in
            CommentSheetView()
